...
module renewal::renew_tests {
    ...
    fun regular_renewal_5_years() {
        ...
        let (mut iotans, mut nft) = prepare_registry(&mut ctx);

        let mut clock = clock::create_for_testing(&mut ctx);

        clock.increment_for_testing(10);

        renew_util(&mut iotans, &mut nft, 5, &clock, &mut ctx);
    ...
    fun fail_to_go_beyond_6_years() {
        ...
        renew_util(&mut iotans, &mut nft, 2, &clock, &mut ctx);
        renew_util(&mut iotans, &mut nft, 4, &clock, &mut ctx);
        abort 1337
    }
    ...
    fun fail_invalid_arg() {
        ...
        renew_util(&mut iotans, &mut nft, 6, &clock, &mut ctx);
        abort 1337
    }
    ...
    fun failed_record_id_missmatch() {
        let mut ctx = tx_context::dummy();
        let (mut iotans, _nft) = prepare_registry(&mut ctx);
        let clock = clock::create_for_testing(&mut ctx);
        let mut nft = nft::new_for_testing(domain::new(utf8(DOMAIN_NAME)), 1, &clock, &mut ctx);
        
        renew_util(&mut iotans, &mut nft, 3, &clock, &mut ctx);
        abort 1337
    }
    ...
    fun failed_expired_record() {
        ...
        renew_util(&mut iotans, &mut nft, 1, &clock, &mut ctx);
        abort 1337
    }
    ...
    fun failed_not_enough_money() {
        ...
        renewal::renew(&mut iotans, &mut nft, 2,coin::mint_for_testing<IOTA>((1 as u64) * REGULAR_PRICE * nanos_per_iota(), &mut ctx), &clock);
        abort 1337
    }
    
    public fun renew_util(iotans: &mut IOTANS, nft: &mut IotansRegistration, no_years: u8, clock: &Clock, ctx: &mut TxContext) {
        renewal::renew(iotans, nft, no_years,coin::mint_for_testing<IOTA>((no_years as u64) * REGULAR_PRICE * nanos_per_iota(), ctx), clock);
    }

    /// Local test to prepare a registry with a domain. 
    /// Authorizes registry, adds domain, and burns admin cap.
    public fun prepare_registry(ctx: &mut TxContext): (IOTANS, IotansRegistration) {

        let mut iotans = iotans::init_for_testing(ctx);
        let mut registry = registry::new_for_testing(ctx);

        let domain = domain::new(utf8(DOMAIN_NAME));        
        iotans::authorize_app_for_testing<Renew>(&mut iotans);

        let clock = clock::create_for_testing(ctx);

        let cap = iotans::create_admin_cap_for_testing(ctx);
        ...
        let config = config::new(
            ...
            REGULAR_PRICE * ::iotans::constants::nanos_per_iota(),
        );

        renewal::setup(&mut iotans, &cap, config);

        let nft = registry.add_record(domain, 1,&clock, ctx);
        iotans::add_registry(&cap, &mut iotans, registry);
    ...
    public fun wrapup_name(nft: IotansRegistration) {
        iota::transfer::public_transfer(nft, @0x2);
    }

    public fun wrapup(iotans: IOTANS) {
