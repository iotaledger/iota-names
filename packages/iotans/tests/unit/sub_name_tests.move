...
#[test_only]
module iotans::sub_name_tests;

use std::string::utf8;
use iota::clock;
use iotans::domain;
use iotans::subdomain_registration as subdomain;
use iotans::iotans_registration;
...
fun test_wrap_and_destroy() {
    ...
    let mut nft = iotans_registration::new_for_testing(
        domain,
        1,
        &clock,
        &mut ctx,
...
#[
    test,
    expected_failure(
        abort_code = iotans::subdomain_registration::ENotSubdomain,
    ),
]
fun try_wrap_non_subdomain() {
    ...
    let nft = iotans_registration::new_for_testing(
...
#[test, expected_failure(abort_code = iotans::subdomain_registration::EExpired)]
...
#[
    test,
    expected_failure(
        abort_code = iotans::subdomain_registration::ENameNotExpired,
    ),
]
