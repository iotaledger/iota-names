...
use iotans::name_record as record;
...
#[test, expected_failure(abort_code = iotans::registry::ERecordNotExpired)]
/// 1. Create a registry, increment clock to 1 year;
...
#[test]
/// Checks that `burn_registration_object` burns `IotansRegistration` object
...
#[test]
/// `burn_registration_object` burns the IotansRegistration object as well as
/// removes the record,
...
#[test]
fun burn_expired_iotans_registration() {
    let mut ctx = tx_context::dummy();
    let (mut registry, mut clock, domain) = setup(&mut ctx);
    let nft = registry.add_record(domain, 1, &clock, &mut ctx);
    // increment the clock to 1 years + grace period
...
#[test, expected_failure(abort_code = iotans::registry::ETargetNotSet)]
/// 1. Create a registry, add a record;
...
#[test, expected_failure(abort_code = iotans::registry::ERecordMismatch)]
/// 1. Create a registry, add a record;
...
#[test, expected_failure(abort_code = iotans::registry::EInvalidDepth)]
...
#[test, expected_failure(abort_code = iotans::registry::ERecordNotFound)]
...
#[test, expected_failure(abort_code = iotans::registry::ENotLeafRecord)]
...
#[test_only]
public fun burn_nfts(mut nfts: vector<IotansRegistration>) {
