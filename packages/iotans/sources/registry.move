...
/// The `IotansRegistration` does not match the `NameRecord`.
...
/// Returns a `IotansRegistration` upon success.
...
public fun burn_registration_object(
    ...
    // First we make sure that the IotansRegistration object has expired.
...
/// an equivalent `IotansRegistration` object.
///
/// Instead, the parent's `IotansRegistration` object is used to manage
...
/// Leaf records do not have any symmetrical `IotansRegistration` object.
...
public fun remove_leaf_record(self: &mut Registry, domain: Domain) {
    ...
    // if it's a leaf record, there's no `IotansRegistration` object.
...
/// Update the `expiration_timestamp_ms` of the given `IotansRegistration` and
/// `NameRecord`. Requires the `IotansRegistration` to make sure that both
...
/// Update the `data` of the given `NameRecord` using a `IotansRegistration`.
...
/// subdomains
/// and don't have their own `IotansRegistration` object Cap. The
/// `IotansRegistration` of the parent
