...
const EInvalidExpirationDate: vector<u8> =
    b"Tried to create a subdomain that expires later than the parent or below the minimum.";
#[error]
const ECreationDisabledForSubdomain: vector<u8> =
    b"Tried to create a subdomain with a parent that is not allowed to do so.";
#[error]
const EExtensionDisabledForSubdomain: vector<u8> =
    b"Tried to extend the expiration of a subdomain which doesn't have the permission to do so.";
#[error]
const ESubdomainReplaced: vector<u8> =
    b"The subdomain has been replaced by a newer NFT, so it can't be renewed.";
#[error]
const EParentChanged: vector<u8> =
    b"Parent for a given subdomain has changed, hence time extension cannot be done.";
#[error]
...
///
/// It then saves the configuration for that child (manage-able by the parent), and returns the IotaNamesRegistration object.
public fun new(
    iota_names: &mut IotaNames,
    parent: &IotaNamesRegistration,
    clock: &Clock,
    ...
    // We register the subdomain (e.g. `subdomain.example.iota`) and return the IotaNamesRegistration object.
    // Aborts with `iota_names::registry::ERecordExists` if the subdomain already exists.
...
public fun extend_expiration(
    ...
    assert!(
        is_extension_allowed(&record_metadata(iota_names, subdomain)),
...
public fun parent(subdomain: &IotaNamesRegistration): ID {
...
fun internal_set_flag(self: &mut IotaNames, subdomain: Domain, key: String, enable: bool) {
...
fun internal_validate_nft_can_manage_subdomain(
    ...
    if (check_creation_auth) {
        ...
        internal_assert_parent_can_create_subdomains(iota_names, parent.domain());
    };

    // validate that the subdomain is valid for the supplied parent.
    config::assert_is_valid_subdomain(&parent.domain(), &subdomain, app_config(iota_names));
}

/// Validate whether a `IotaNamesRegistration` object is eligible for creating a subdomain.
...
fun internal_assert_parent_can_create_subdomains(self: &IotaNames, parent: Domain) {
...
fun internal_create_subdomain(
    ...
    // attach the `ParentID` to the IotaNamesRegistration, so we validate that the parent who created this subdomain
