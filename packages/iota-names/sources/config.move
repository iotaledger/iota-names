...
/// Module holding the application configuration for the V1 of the IotaNames
...
/// - attached to the IotaNames object directly and can only be *read* by other
/// parts of the system;
///
/// Notes:
...
public fun assert_valid_user_registerable_domain(domain: &Domain) {
    ...
    assert!(domain.tld() == &constants::iota_tld(), EInvalidTld);
...
/// TODO: revisit, are we sure we can't use less than 1 IOTA?
fun check_price(price: u64) {
    assert!(
        constants::nanos_per_iota() <= price
            && price <= constants::nanos_per_iota() * 1_000_000,
        EInvalidPrice,
    );
}
