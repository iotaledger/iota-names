...
import { getClient, setupIotaClient, TestToolbox } from './toolbox';
...
describe('IOTA Names Localnet Integration Tests', () => {
    ...
    describe('Name Validation and Pricing', () => {
        it('should calculate correct prices for different name lengths', async () => {
            const threeLetter = await client.calculatePrice({ name: 'abc.iota', years: 1 });
            const fourLetter = await client.calculatePrice({ name: 'test.iota', years: 1 });
            const fiveLetter = await client.calculatePrice({ name: 'hello.iota', years: 1 });
    ...
    describe('Name Registration Flow', () => {
        ...
        it('should handle registration with insufficient funds', async () => {
            const tx = new Transaction();
            const iotaNamesTx = new IotaNamesTransaction(client, tx);

            const uniqueName = `insufficient-${Date.now()}.iota`;

            // Split a very small amount - insufficient for 4-letter name registration (needs 0.1 IOTA)
            const [coinInput] = iotaNamesTx.transaction.splitCoins(iotaNamesTx.transaction.gas, [
                1000000n, // 0.001 IOTA - way less than the 0.1 IOTA needed for 'test' registration
            ]);
            ...
            // Set a reasonable gas budget for the transaction execution, but the payment coin is insufficient
            tx.setGasBudget(1000000n); // 1 IOTA for gas
            ...
            // The transaction should fail due to insufficient payment for registration
