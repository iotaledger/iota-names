...
export class IotansClient {
	#client: IotaClient;
	constants: Constants = {};

	constructor(config: IotansClientConfig) {
	...
	/**
	 * Returns the price list for IOTANS names.
	 */
	async getPriceList(): Promise<IotansPriceList> {
		if (!this.constants.iotansObjectId) throw new Error('IOTANSobject ID is not set');
		if (!this.constants.iotansPackageId) throw new Error('Price list config not found');

		const priceList = await this.#client.getDynamicFieldObject({
			parentId: this.constants.iotansObjectId,
			name: {
				type: getConfigType(
					this.constants.iotansPackageId.v1,
					getPricelistConfigType(this.constants.iotansPackageId.v1),
				),
	...
	async getRenewalPriceList(): Promise<IotansPriceList> {
		if (!this.constants.iotansObjectId) throw new Error('IOTANSobject ID is not set');
		if (!this.constants.iotansPackageId) throw new Error('Price list config not found');
	...
	async getNameRecord(name: string): Promise<NameRecord | null> {
		...
		if (!this.constants.iotansPackageId) throw new Error('IOTANSpackage ID is not set');
		...
		const nameRecord = await this.#client.getDynamicFieldObject({
			...
			name: {
				type: getDomainType(this.constants.iotansPackageId.v1),
				value: normalizeIOTANSName(name, 'dot').split('.').reverse(),
			},
		});
	 ...
	 * It expects a domain name, the number of years and a `IotansPriceList` object,
	 * as returned from `iotansClient.getPriceList()` function, or `iotans.getRenewalPriceList()` function.
	 *
	 * It throws an error:
	 * 1. if the name is a subdomain
	 * 2. if the name is not a valid IOTANS name
	...
	calculatePrice({
		...
		name: string;
		years: number;
		priceList: IotansPriceList;
	}) {
		if (!isValidIOTANSName(name)) throw new Error('Invalid IOTANS name');
		validateYears(years);
		if (isSubName(name)) throw new Error('Subdomains do not have a registration fee');

		const length = normalizeIOTANSName(name, 'dot').split('.')[0].length;
