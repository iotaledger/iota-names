...
```sh
cargo install --locked --git https://github.com/iotaledger/mvr --branch release mvr
```

### From binaries

Download the correct binary file for your OS and architecture from the [MVR GitHub release page](https://github.com/iotaledger/mvr/releases/latest):
...
1. Clone the `mvr` repository.
    ```sh
    git clone https://github.com/iotaledger/mvr.git
    ```
...
Use `iota move build` as usual when building Move packages. The command automatically invokes MVR CLI to resolve dependencies and build your project.
