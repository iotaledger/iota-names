...
`IotansTransaction` is the client used similar to `Transaction`, and helps in building a transaction.
...
const register = async (name: string, years: number) => {
    ...
    // Pass in the transaction block & the app's global IotansClient.
    const iotansTransaction = new IotansTransaction(iotansClient, transaction);

	// Build the transaction to register the name.
	const nft = iotansTransaction.register({
		name,
		years,
...
const renew = async (nftId: string, name: string, years: number) => {
    ...
    const priceList = await iotansClient.getRenewalPriceList();
    // Create a transaction block as usual in your PTBs.
    const transaction = new Transaction();
    // Pass in the transaction block & the app's global IotansClient.
    const iotansTransaction = new IotansTransaction(iotansClient, transaction);
...
const setTargetAddress = async (nftId: string, address: string) => {
    ...
    iotansTransaction.setTargetAddress({
        nftId,
        address,
        isSubname: false,
    });
...
const setDefault = async (name: string) => {
    ...
    iotansTransaction.setDefault(name);
...
const createSubname = async (subName: string, parentNftId: string, expirationMs: number) => {
	...
	const subNameNft = iotansTransaction.createSubName({
...
const extendExpiration = async (nftId: string, expirationMs: number) => {
    ...
    iotansTransaction.extendExpiration({
        nft: nftId,
        expirationTimestampMs: expirationMs,
    });
...
const createLeafSubname = async (name: stringify, parentNftId: string, targetAddress: string) => {
    ...
    iotansTransaction.createLeafSubName({
        // The NFT of the parent
        parentNft: parentNftId,
...
const removeLeafSubname = async (name: string, parentNftId: string) => {
    ...
    iotansTransaction.removeLeafSubName({
        // The NFT of the parent
        parentNft: parentNftId,
...
The following code snippet registers a IOTANS name, sets its target address, and sets it as the default name for the target address in a single PTB. You could also add transaction commands to do even more in the same PTB if you wanted to, like create subnames and so on, but that is beyond the scope of this example.
