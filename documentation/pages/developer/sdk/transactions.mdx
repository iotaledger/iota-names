...
const register = async (name: string, years: number) => {
    ...
    const iotaNamesTransaction = new SuinsTransaction(iotaNamesClient, transaction);
    // Specify the coin type used for the transaction
    const coinConfig = iotaNamesClient.config.coins.IOTA;

    // priceInfoObjectId is required for IOTA/NS
    ...
    const nft = iotaNamesTx.register({
        ...
        coinConfig,
        coin,
        priceInfoObjectId, // Only required for IOTA/NS
    });
...
const setUserData = async (nft: string, avatar: string, contentHash: string) => {
    ...
    iotaNamesTransaction.setUserData({
        nft,
...
const burnExpired = async (nftId: string) => {
    ...
    iotaNamesTransaction.burnExpired({
        nft: nftId,
        isSubname: false,
    });
