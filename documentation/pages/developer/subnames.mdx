...
| --- | --- | --- |
| Has NFT | ✅ Yes, <ul style={{listStyle: 'disc', paddingLeft: 1 + 'rem'}}><li>`IotaNamesRegistration` for SLD*</li><li>`SubdomainRegistration` for nested names</li></ul> | ❌ No, parent's NFT acts as the capability |
| Can create children (node/leaf) | ✅ Yes, provided the name is an SLD* or the parent allows it | ❌ No |
| Expiration | ✅ Yes, IOTA-Names determines expiration for SLDs*, otherwise the parent determines | ❌ No, the parent's expiration determines the leaf expiration |
...
*SLD - Second-level domain (for instance, example in example.iota)*
