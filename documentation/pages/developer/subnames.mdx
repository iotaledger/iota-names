...
| --- | --- | --- |
| Has NFT | ✅ Yes, <ul style={{listStyle: 'disc', paddingLeft: 1 + 'rem'}}><li>`IotansRegistration` for SLD*</li><li>`SubDomainRegistration` for nested names</li></ul> | ❌ No, parent's NFT acts as the capability |
| Can create children (node/leaf) | ✅ Yes, provided the name is an SLD* or the parent allows it | ❌ No |
| Expiration | ✅ Yes, IOTANS determines expiration for SLDs*, otherwise the parent determines | ❌ No, the parent's expiration determines the leaf expiration |
...
| Transfer ownership | ✅ Yes, the NFT determines node name ownership | ❌ No, IOTANS preserves ownership for the active parent name holder |
