...
The maximum nesting depth is **eight** levels. The total depth, including the SLN and TLN, means the maximum total depth is 10 levels.
...
| --- | --- | --- |
| Has NFT | ✅ Yes, <ul style={{listStyle: 'disc', paddingLeft: 1 + 'rem'}}><li>`NameRegistration` for SLN*</li><li>`SubnameRegistration` for nested names</li></ul> | ❌ No, parent's NFT acts as the capability |
| Can create children (node/leaf) | ✅ Yes, provided the name is an SLN* or the parent allows it | ❌ No |
| Expiration | ✅ Yes, IOTA-Names determines expiration for SLNs*, otherwise the parent determines | ❌ No, the parent's expiration determines the leaf expiration |
| Extend expiration | ✅ Yes, you can extend expiration if parent rules allow it (or you can pay for it for SLN*) | ✅ Yes, leaf names do not expire |
...
| Transfer ownership | ✅ Yes, the NFT determines node name ownership | ❌ No, IOTA-Names preserves ownership for the active parent name holder |

*SLN - Second-level name (for instance, example in example.iota)*
