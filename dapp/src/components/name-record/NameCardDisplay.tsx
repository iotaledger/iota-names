...
interface NameCardDisplayProps extends NftDisplayProps {
    ...
    return (
        ...
        >
            <div className="w-full h-full flex flex-col relative rounded-xl overflow-hidden">
                {staticImageSrc ? (
                    <StaticImageDisplay src={staticImageSrc} alt={name} />
                ) : (
                    <AvatarDisplay
                        name={name}
                        badge={badge}
...
type AvatarDisplayProps = Pick<NameCardDisplayProps, 'name' | 'badge' | 'button' | 'fallbackUrl'>;

export function AvatarDisplay({ name, badge, button, fallbackUrl }: AvatarDisplayProps) {
...
function StaticImageDisplay({
    src,
    alt,
}: Pick<React.ImgHTMLAttributes<HTMLImageElement>, 'src' | 'alt'>) {
    return (
        <img className="absolute inset-0 w-full h-full -z-[1] object-cover" src={src} alt={alt} />
    );
}
