...
import type { DataFields, IotaNamesContract, NameObject, NetworkType } from './types/objects';
...
class IotaNamesClient {
	...
	contractObjects: IotaNamesContract | undefined;
	networkType: NetworkType | undefined;

	constructor(
		iotaClient: IotaClient,
		options?: {
			contractObjects?: IotaNamesContract;
	...
	async getIotaNamesContractObjects() {
		if ((this.contractObjects as IotaNamesContract)?.packageId) return;
		...
		try {
			...
			throw new Error(`Error getting IOTA-Names contract objects, ${(error as Error).message}`);
		}
	...
	protected async getNameData(dataObjectId: string, fields: DataFields[] = []) {
		...
		const fulfilledData = data.filter((e) => e.status === 'fulfilled') as PromiseFulfilledResult<
			[string, unknown]
		>[];
	...
	async getNameObject(
		...
		await this.getIotaNamesContractObjects();

		const registryResponse = await this.getDynamicFieldObject(
			(this.contractObjects as IotaNamesContract).registry,
