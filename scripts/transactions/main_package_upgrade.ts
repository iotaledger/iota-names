...
const mainPackageUpgrade = async () => {
	...
	// we execute this on `setup/package.json` so we go one level back, access packages folder -> iotans -> upgrade.
	// we go from scripts/(base)/packages/iotans, we run the upgrade and then we save the transaction data
	// to iotans/..(packages)/..(base)/scripts/tx/tx-data.txt
	execSync(`cd $PWD/../packages/iotans && ${upgradeCall} > $PWD/../../scripts/tx/tx-data.txt`);
};
