...
import { SuiPriceServiceConnection, SuiPythClient } from '@pythnetwork/pyth-iota-js';
...
export const getPriceInfoObject = async (tx: Transaction, feed: string) => {
	...
	const client = new SuiPythClient(iotaClient, pythStateId, wormholeStateId);
...
export const calculatePriceAfterDiscount =
	(paymentIntent: TransactionObjectArgument, paymentType: string) => (tx: Transaction) => {
		return tx.moveCall({
			...
			arguments: [tx.object(config.iotaNames), paymentIntent],
			typeArguments: [paymentType],
		});
	};
...
// exampleRegistration(
// 	'ajjdfksadsskdddddsddsssssddddddsd.iota', // Domain to register
// 	4,
// 	config.coins.USDC,
// 	{
...
// exampleRegistration(
// 	'ajjdfksadsskdddddssssdssssdssddddddsd.iota', // Domain to register
// 	4,
// 	config.coins.USDC,
// 	{
...
// exampleRegistration('ajasdsssssmmdaaaadsafsaaaaaddsssaaasd.iota', 2, config.coins.SUI);

/*  Example registration using NS */
// exampleRegistration('ajasddsafaaaddsssddssaaaasd.iota', 3, config.coins.NS, {
